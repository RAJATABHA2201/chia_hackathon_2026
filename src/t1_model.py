...
# Per-access SRAM energy scales with the square root of the on-chip SRAM
# (scratchpad + accumulator), the usual first-order bitline/wordline-length
# law: ENERGY_PJ_PER_SRAM_BYTE is the cost at SRAM_REF_BYTES.
#
# The FLOOR used to be the reference itself (`max(1.0, bytes / 256 KB)`), so
# every design at or below 256 KB paid the full 256 KB access cost. Every
# admitted design after V1's iteration 5 sat below it (sp 256 -> 128 -> 64 KB,
# acc 64 -> 32 -> 16 KB), and all of them scored IDENTICAL energy: the model
# could not see the change the search was making. Below the floor, fixed
# periphery (sense amps, decoders, IO) dominates and the square-root law stops
# holding, so a floor is still needed, just not at the reference.
# 32 KB: 0.354x the reference cost at the floor, i.e. about 2.8x cheaper per
# access than a 256 KB array, which is as far as the square-root law is
# trusted here.
SRAM_REF_BYTES = 256 * 1024
SRAM_FLOOR_BYTES = 32 * 1024


def sram_access_scale(sram_bytes: int) -> float:
    """Per-access SRAM energy multiplier relative to SRAM_REF_BYTES."""
    return math.sqrt(max(SRAM_FLOOR_BYTES, sram_bytes) / SRAM_REF_BYTES)
...
def predict(s: DesignState, w: Workload) -> Prediction:
    ...
    sram_scale = sram_access_scale(sram_b)
...
class EnergyReport:
    ...
    breakdown: dict
    # "counter"  -- ZBU_SKIPPED_ROWS came from the RTL
    # "derived"  -- counted exactly over the workload's own A data (zbu_model)
    # "off"      -- the ZBU is disabled, or nothing to report
    zbu_source: str = "off"
...
def workload_header_path(state) -> str | None:
    """Path to the generated header this design's kernel builds against.

    `DesignState.workload` is the stem ("dnn512"); prep_matrices.py emits
    `workload/generated/spmm_<stem>.h` under constants.WORKLOAD_DIR (the
    package's workload/, one level above src/). Returns None when
    the header is absent, so a caller can degrade rather than raise -- a
    missing workload file is a setup problem, not a reason to fail scoring.
    """
    import os
    stem = getattr(state, "workload", None)
    if not stem:
        return None
    from constants import WORKLOAD_DIR
    path = os.path.join(WORKLOAD_DIR, "generated", f"spmm_{stem}.h")
...
def energy_report(s: DesignState, m, w: Workload, period_ns: float) -> EnergyReport:
    ...
    sram_scale = sram_access_scale(sram_bytes_exact(s))
    sram_bytes_charged = macs_issued * 2 * INPUT_BYTES

    # One suppressed row read avoids a DIM-wide vector of the A operand.
    #
    # PREFER THE RTL COUNTER. When `ZBU_SKIPPED_ROWS` exists it is a
    # measurement and wins outright. It does not exist today -- the only
    # SparseCraft id in `CounterFile.scala` is `MAC_GATED_TOTAL` -- so the
    # lookup returns 0 and this whole term silently vanishes. That is why T-B
    # has never been admitted: its cost is synthesised and charged, its
    # benefit is invisible, so it is strictly dominated and the agent is right
    # to reject it.
    #
    # FALL BACK TO AN EXACT DERIVATION. The number of all-zero granules is a
    # property of the workload, and the harness generated the workload, so it
    # can be counted rather than guessed -- see zbu_model. This is a MODEL of
    # what the hardware would skip, not a measurement of what it did, and
    # `zbu_source` says which one produced the number.
    #
    # It deliberately credits ENERGY only. The RTL's `sc_skip` gates the
    # scratchpad read while leaving `ren`, `q.io.enq.valid` and
    # `io.read.req.ready` stock, so the pipeline slot is still consumed and no
    # cycles are saved. Crediting cycles here would invent a benefit the
    # hardware does not deliver.
    zbu_rows = int(m.counters.get("ZBU_SKIPPED_ROWS", 0) or 0)
    zbu_source = "counter" if zbu_rows else "off"
    if not zbu_rows and getattr(s, "zbu_enable", False):
        try:
            import zbu_model
            hdr = workload_header_path(s)
            if hdr:
                zbu_rows, _info = zbu_model.derived_zbu_rows(
                    hdr, int(getattr(s, "granule_size", w.dim)),
                    str(getattr(s, "zbu_operand", "A")), macs_issued)
                zbu_source = "derived" if zbu_rows else "off"
        except Exception:                                    # noqa: BLE001
            # A derivation that cannot run must not fail the iteration: the
            # term simply stays 0, exactly as before, and zbu_source says so.
            zbu_rows, zbu_source = 0, "off"
