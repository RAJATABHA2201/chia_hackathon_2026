...
node type in configs/cluster.yaml. The expensive ones wrap CHIA's existing Chipyard
...
# --------------------------------------------------------------------------
# Pattern name -> the P_* code the (archived) attn_prefill.c switched on. An unknown name
...
def parse_params_scala(text: str) -> dict:
    ...
    for name in ("gate_enable", "zbu_enable", "dense_mode", "x_resident"):
