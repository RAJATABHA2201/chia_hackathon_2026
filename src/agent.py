...
from constants import PROMPTS_DIR as _PROMPTS_DIR

# ../prompts relative to src/. From constants, so the layout is defined once.
...
def resolve_includes(text: str, _seen: tuple = ()) -> str:
    """Expand ``{{include: shared/foo.md}}`` lines, recursively.

    WHY THIS EXISTS. Four prompts have to agree on the same facts -- the
    writable set, the host's quirks, the output schema -- and in V1 each
    restates them. Restated facts drift, and one of them is also enforced in
    code (``t0.check_patch_scope``), so a prompt that drifts from the enforcer
    teaches the agent something false about what it is allowed to do. An
    include has exactly one definition.

    The directive must be ALONE on its line: it is replaced by a block of
    prose, and splicing a paragraph into the middle of a sentence produces
    text no one proofread.

    Cycles raise rather than recursing to a stack overflow, and the error
    names the chain -- a self-including prompt is a typo, and the fix is
    obvious only if you can see which files are involved.
    """
    def _sub(m):
        rel = m.group(1)
        if rel in _seen:
            chain = " -> ".join(_seen + (rel,))
            raise ValueError(f"circular prompt include: {chain}")
        path = PROMPTS_DIR / rel
        if not path.is_file():
            available = ", ".join(sorted(
                str(q.relative_to(PROMPTS_DIR)) for q in PROMPTS_DIR.rglob("*.md")))
            raise FileNotFoundError(
                f"prompt include {rel!r} not found under {PROMPTS_DIR}. "
                f"Available: {available}")
        return resolve_includes(path.read_text().rstrip("\n"), _seen + (rel,))
    return _INCLUDE_RE.sub(_sub, text)


def read_prompt(prompt_file: str) -> str:
    """Read a prompt and expand its includes. No ``${}`` substitution."""
    path = PROMPTS_DIR / prompt_file
    if not path.is_file():
        available = ", ".join(sorted(
            str(q.relative_to(PROMPTS_DIR)) for q in PROMPTS_DIR.rglob("*.md"))) or "none"
        raise FileNotFoundError(
            f"no prompt {prompt_file!r} in {PROMPTS_DIR}. Available: {available}")
    return resolve_includes(path.read_text())


def compose(system: str, strategy: list | None = None) -> str:
    """Build a system message in PROMPT-CACHE ORDER.

    Stable first, volatile last: the role and its shared fragments are
    immutable for the whole run, while the strategy set changes only when the
    diagnosed bottleneck moves. CHIA's own token accounting shows cache-READ
    tokens dominating spend, so an ordering that keeps the long prefix
    byte-stable is worth more than it looks.
    """
    parts = [read_prompt(system)]
    for rel in (strategy or []):
        parts.append(read_prompt(rel))
    return "\n\n".join(parts)
...
def load_prompt(prompt_file: str, **values: object) -> str:
    ...
    path = PROMPTS_DIR / prompt_file
    try:
        # Includes first: a shared fragment may itself carry ${} placeholders,
        # and they must be visible to the mismatch check below.
        raw = resolve_includes(path.read_text())
...
def make_llm(system_prompt_file: str, backend: str | None = None,
    ...
    """
    name, spec = provider(backend)
    system_message = read_prompt(system_prompt_file)
