...
decided it.
    ...
    python scripts/check_setup.py              # table
    python scripts/check_setup.py --json       # machine-readable, for the deck
    python scripts/check_setup.py --quick      # host + image presence only, no container starts
...
# scripts/ -> the sparsecraft-v2 checkout, and the source and config dirs in
# it. PROJECT is the directory ABOVE the checkout, as before: it is only used
# to report free disk on the volume that holds runs/ and the podman store.
PKG = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC = os.path.join(PKG, "src")
CONFIGS = os.path.join(PKG, "configs")
PROJECT = os.path.dirname(PKG)
...
def probe_wiring() -> dict:
    cluster = os.path.join(CONFIGS, "cluster.yaml")
    ...
    for fname in ("nodes.py", "synth_node.py", "diff_nodes.py", "agent.py"):
        ...
        if not os.path.exists(path):
            # A missing module used to be skipped silently, so after a file
            # moved this probe reported "all wired" while checking nothing.
            requested.setdefault("?MISSING-SOURCE?", []).append(fname)
            continue
        src = open(path).read()
            ...
            for km in re.finditer(r'(?:"([A-Za-z_][\w]*)"|R_([A-Z_]+))\s*:', body):
                ...
                if name is None:
                    ...
                    cm = re.search(rf'^{const}\s*=\s*"([^"]+)"',
                                   open(os.path.join(SRC, "constants.py")).read(), re.M)
...
def main() -> int:
    ...
    print()
    w = report["wiring"]
    print("resources requested by the code vs advertised by configs/cluster.yaml:")
