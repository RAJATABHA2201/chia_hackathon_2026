...
configured provider and prints what came back.
    ...
    python scripts/check_llm.py                 # call the configured backend
    python scripts/check_llm.py --list          # every backend and whether it is ready
    python scripts/check_llm.py --backend gemini --model gemini-2.5-flash
...
def main() -> int:
    ...
    try:
        agent.make_llm("system/microarchitect.md", backend=name, model=model)
    ...
    if ok:
        ...
        print(f"      scripts/run.sh --iters 5")
        return 0
    ...
    print(f"      other backends: python scripts/check_llm.py --list")
    return 1
