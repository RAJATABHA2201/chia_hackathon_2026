...
separate entry point.
    ...
    python smoke_agent.py                    # uses the configured backend
    python smoke_agent.py --backend custom   # against a fake server
...
def main() -> int:
    ...
    if not info["ready"]:
        print(f"ABORT: backend {info['backend']!r} is not ready. "
              f"Run: python check_llm.py --list")
        return 2
    ...
    for needed in ("llm", C.R_CHIPYARD, C.R_HEAD_LOCAL):
        if res.get(needed, 0) <= 0:
            print(f"ABORT: no worker advertises {needed!r}. "
                  f"Is the cluster up with this cluster.yaml?")
            return 2
    ...
    tools = []
    llm = agent.make_llm("propose.md", backend=args.backend, model=args.model)
    try:
